...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-9">
            <div class="card">
                <div class="card-header"  >      {{ $section }} الإعدادات</div>
                <div class="card-body">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                @elseif($setting->type == 'textarea')
                                    <textarea name="value[{{ $loop->index }}]" id="value"
                                     class="form-control" cols="30" rows="10">{{ $setting->value }}</textarea>
                                @elseif($setting->type == 'image')
                                    <input type="file" name="value[{{ $loop->index }}]" id="value"
                                     class="form-control">
                                @elseif($setting->type == 'select')
                                    {!! Form::select('value[' . $loop->index . ']', 
                                        explode('|', $setting->details) , $setting->value , ['id' => 'value', 'class' => 'form-control']) !!}
                                @elseif($setting->type == 'checkbox')
                                    {!! Form::checkbox('value[' . $loop->index . ']', 1, 
                                        $setting->value == 1 ? true : false , ['id' => 'value', 
                                        'class' => 'styled']) !!}
                                @elseif($setting->type == 'radio')
                                    {!! Form::radio('value[' . $loop->index . ']', 1, $setting->value == 1 ? true : false , ['id' => 'value', 'class' => 'styled']) !!}
                                @endif

                                <input type="hidden" name="key[{{ $loop->index }}]" 
                                id="key" 
                                class="form-control" value="{{ $setting->key }}" readonly>
                                <input type="hidden" name="id[{{ $loop->index }}]" id="key" 
                                class="form-control" value="{{ $setting->id }}" readonly>
                    ...
                    <div class="text-right">
                        {!! Form::button('إرسال ', ['type' => 'submit', 'class' => 'btn btn-primary']) !!}
                    </div>
                    {!! Form::close() !!}
                </div>
            </div>
